...
public class AnswerController {
    ...
    public AnswerController(AnswerService service, AnswerMapper mapper, JwtUtils jwtUtils) {
    ...
    // 답변 등록
    @ApiOperation(value = "답변을 등록하는 메서드", notes = "<b style=\"font-size: 18px;\"> JWT(필수) </b> questionId, body를 사용해서 답변을 생성한다")
    @ApiResponses({
    ...
    public ResponseEntity postAnswer(@Valid @RequestBody AnswerPostDto answerPostDto) {
        answerPostDto.setMemberId(jwtUtils.getMemberId());
    ...
    // 답변 수정
    @ApiOperation(value = "하나의 답변을 수정하는 메서드", notes = "<b style=\"font-size: 18px;\"> JWT(필수) </b> body에 변경사항을 적용해 답변을 수정한다")
    @ApiResponses({
    ...
    public ResponseEntity patchAnswer(@PathVariable("answer-id") @Positive long answerId,
        ...
        answerPatchDto.setMemberId(jwtUtils.getMemberId());
    ...
    // 답변 삭제
    @ApiOperation(value = "하나의 답변을 삭제하는 메서드", notes = "<b style=\"font-size: 18px;\"> JWT(필수) </b> answerId를 사용해 답변을 삭제한다")
    ...
    public ResponseEntity deleteAnswer(@PathVariable("answer-id")@Positive long answerId) {
        service.deleteAnswer(answerId, jwtUtils.getMemberId());
