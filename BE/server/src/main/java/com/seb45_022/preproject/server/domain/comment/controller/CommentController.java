...
public class CommentController {
    ...
    public CommentController(CommentService service, CommentMapper mapper, JwtUtils jwtUtils) {
    ...
    // 댓글 등록
    @ApiOperation(value = "댓글을 등록하는 메서드", notes = "<b style=\"font-size: 18px;\"> JWT(필수) </b> answerId, memberId, body를 사용해서 질문을 생성한다")
    @ApiResponses({
    ...
    public ResponseEntity postComment(@Valid @RequestBody CommentPostDto commentPostDto) {
        commentPostDto.serMemberId(jwtUtils.getMemberId());
    ...
    // 댓글 수정
    @ApiOperation(value = "하나의 댓글을 수정하는 메서드", notes = "<b style=\"font-size: 18px;\"> JWT(필수) </b> body의 변경사항을 적용해 댓글을 수정한다")
    @ApiResponses({
    ...
    public ResponseEntity patchComment(@PathVariable("comment-id") @Positive long commentId,
        ...
        commentPatchDto.setMemberId(jwtUtils.getMemberId());
    ...
    // 댓글 삭제
    @ApiOperation(value = "하나의 댓글을 삭제하는 메서드", notes = "<b style=\"font-size: 18px;\"> JWT(필수) </b> commentId를 이용해 댓글을 삭제한다")
    ...
    public ResponseEntity deleteComment(@PathVariable("comment-id")@Positive long commentId) {
        ...
        service.deleteComment(commentId, jwtUtils.getMemberId());
